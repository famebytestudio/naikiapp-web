...
function ClaimCard({ claim, onStatusUpdate, updatingId }) {
	...
	return <article className="claim-card"><div className="claim-card-heading"><div><p className="food-type">{claim.type}</p><h2>{claim.food}</h2><p className="donor">{claim.quantity} {claim.unit} / {claim.kg} kg</p></div><span className={`claim-status ${claim.status}`}>{claim.status.replace('_', ' ')}</span></div><div className="claim-details"><span>Claimed {formatDate(claim.claimedAt)}</span><span>Pickup {formatDate(claim.pickupStart)}</span><span>{claim.area}, {claim.city}</span></div><div className="claim-card-actions">{nextStatus && <button className="claim-button" type="button" onClick={() => onStatusUpdate(claim.id, nextStatus)} disabled={updatingId === claim.id}>{updatingId === claim.id ? 'Updating...' : nextStatus === 'picked_up' ? 'Mark picked up' : 'Mark delivered'}</button>}<Link to={`/ngo/listings/${claim.id}`} className="map-link">View donation <span aria-hidden="true">&#8594;</span></Link></div></article>
}
